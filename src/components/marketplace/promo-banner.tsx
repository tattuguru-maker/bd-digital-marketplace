...
export function PromoBannerCard({ banner }: { banner: PromoBanner }) {
  return (
    <Link
      href={banner.href}
      className="glass-card group relative block overflow-hidden p-5 transition hover:border-white/20"
    >
      ...
      {/* Big watermark text */}
      <div className="pointer-events-none absolute -right-2 bottom-0 select-none text-right font-display text-[64px] font-black uppercase leading-none tracking-tight text-white/10 md:text-[88px]">
        {banner.artLabel}
      </div>
      <div className="relative flex min-h-[140px] flex-col justify-between gap-3">
        ...
        <div>
          <div className="font-display text-lg font-bold leading-tight text-white md:text-xl">
            {banner.title}
          </div>
          <div className="mt-1 text-[12.5px] text-white/75">{banner.subtitle}</div>
        </div>
      </div>
    </Link>
  );
